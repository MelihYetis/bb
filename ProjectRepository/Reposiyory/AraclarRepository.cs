...
{
    public class AraclarRepository<T> : ImyReposiory<T> where T : class
    {
        protected ContextDataModel _context { get; set; }
        protected DbSet<T> _dbSet { get; set; }

        public AraclarRepository(ContextDataModel dbContext)
        {
        ...
        public IList<Dosyalar> ListDosyalar()
        {
            var query = (from i in _context.Dosyalar select i).ToList();
            return query;
        }
    }
}
